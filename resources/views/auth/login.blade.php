...
<head>
  ...
  <title>Login | Mriki Project</title>
  ...
  <style>
    ...
    *{ box-sizing: border-box; }
    ...
    .mriki-checkbox{ accent-color: var(--brand-red); }
  ...
  <div class="mriki-bg">
    ...
    <section class="mriki-card">
      <div class="mriki-card__header">
        ...
        <div>
          <h1 class="mriki-card__title">Login</h1>
          <p class="mriki-card__subtitle">Masuk untuk lanjut booking.</p>

          @if (session('status'))
            <div class="mriki-status">{{ session('status') }}</div>
          @endif
        </div>
      </div>
      ...
      <div class="mriki-card__body">
        <form method="POST" action="{{ route('login') }}">
          ...
          <div style="margin-top:14px;">
            <label style="display:flex; align-items:center; gap:10px; margin:0; font-weight:700;">
              <input id="remember_me" type="checkbox" class="mriki-checkbox" name="remember">
              <span style="color:rgba(255,255,255,.86);">Remember me</span>
            </label>
          </div>
          ...
          <div class="mriki-row">
            <div style="display:flex; gap:12px; flex-wrap:wrap; align-items:center;">
              @if (Route::has('password.request'))
                <a class="mriki-link" href="{{ route('password.request') }}">Lupa password?</a>
              @endif

              @if (Route::has('register'))
                <a class="mriki-link" href="{{ route('register') }}">Register</a>
              @endif
            </div>

            <button type="submit" class="mriki-btn">Log in</button>
          </div>
