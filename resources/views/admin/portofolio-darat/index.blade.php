...
@section('title', 'Kelola Portofolio Jasa Darat')

@section('content')
<div class="flex items-center justify-between mb-4">
  <div>
    <h1 class="text-xl font-extrabold text-slate-900">Kelola Portofolio Jasa Darat</h1>
    <p class="text-sm text-slate-500">Project jasa darat</p>
  </div>

  <div class="flex items-center gap-2">
    {{-- ✅ tombol: ke halaman layanan index --}}
    ...
    {{-- ✅ tombol lama, ganti label: tambah portofolio --}}
    <a href="{{ route('admin.portofolio-darat.create') }}"
        ...
        <td class="p-3">
          <div class="flex justify-end gap-2">
            <a class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 transition"
               href="{{ route('admin.portofolio-darat.show', $item) }}">Detail</a>
            <a class="px-3 py-1.5 rounded-lg bg-amber-100 hover:bg-amber-200 transition"
               href="{{ route('admin.portofolio-darat.edit', $item) }}">Edit</a>
            <form method="POST" action="{{ route('admin.portofolio-darat.destroy', $item) }}"
