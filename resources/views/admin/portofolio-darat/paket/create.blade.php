...
<head>
  ...
  <title>Tambah Paket Layanan Darat</title>
  @vite(['resources/css/app.css','resources/js/app.js'])
</head>
<body class="bg-slate-50 text-slate-900">
  <div class="max-w-2xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-extrabold">Tambah Paket</h1>
    ...
    <div class="mt-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
      <form method="POST" action="{{ route('admin.paket-layanan-darat.store') }}" class="space-y-4">
        @csrf
        ...
        <div>
          ...
          <select name="layanan_id" class="mt-1 w-full px-3 py-2 rounded-xl border border-slate-200">
            @foreach($layananList as $l)
              <option value="{{ $l->id }}" @selected(old('layanan_id') == $l->id)>{{ $l->nama }}</option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <textarea name="deskripsi" rows="4"
                    class="mt-1 w-full px-3 py-2 rounded-xl border border-slate-200"
                    placeholder="Contoh: Durasi 3 Jam, File Master JPG/PNG, 1 Crew Photographer..."></textarea>
        </div>
