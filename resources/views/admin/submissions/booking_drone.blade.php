...
@section('title', 'Kelola Booking Drone - Admin')
...
<div class="max-w-6xl mx-auto">
  <div class="mb-6 flex justify-between items-center">
    <div>
      <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Daftar Booking Jasa Drone</h1>
      <p class="text-sm text-slate-500 mt-1">Daftar pengajuan booking jasa drone udara dari client.</p>
    </div>
  </div>
      ...
      <table class="w-full text-left border-collapse text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse($bookings as $b)
            @include('admin.submissions.partials.booking_drone_row', ['b' => $b])
          @empty
            <tr class="empty-state-row">
              <td colspan="8" class="px-6 py-12 text-center text-slate-500">
                <div class="flex flex-col items-center justify-center gap-2">
                  ...
                  <span class="font-medium">Belum ada pengajuan booking drone.</span>
                </div>
              </td>
            </tr>
          @endforelse
...
<script>
  ...
  // REAL-TIME CHECK FOR NEW BOOKINGS
  ...
  setInterval(function() {
    ...
    fetch(`{{ route('admin.submissions.fetch-new') }}?type=booking_drone&last_id=${lastBookingId}`)
      .then(res => res.json())
      .then(data => {
