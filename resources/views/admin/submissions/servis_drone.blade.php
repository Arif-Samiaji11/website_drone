...
@section('title', 'Kelola Servis Drone - Admin')
...
<div class="max-w-6xl mx-auto">
  <div class="mb-6 flex justify-between items-center">
    <div>
      <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Daftar Pengajuan Servis Drone</h1>
      <p class="text-sm text-slate-500 mt-1">Daftar permintaan servis & perbaikan unit drone dari client.</p>
    </div>
  </div>
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-left border-collapse text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse($servis as $s)
            @include('admin.submissions.partials.servis_drone_row', ['s' => $s])
          @empty
            <tr class="empty-state-row">
              <td colspan="8" class="px-6 py-12 text-center text-slate-500">
                <div class="flex flex-col items-center justify-center gap-2">
                  ...
                  <span class="font-medium">Belum ada pengajuan servis unit drone.</span>
                </div>
              </td>
            </tr>
          @endforelse
...
<script>
  ...
  // REAL-TIME CHECK FOR NEW SERVIS REQUESTS
  let lastBookingId = {{ $servis->first()->id ?? 0 }};
  const tableBody = document.querySelector('tbody');
  
  setInterval(function() {
    if (lastBookingId <= 0) return;
    
    fetch(`{{ route('admin.submissions.fetch-new') }}?type=servis_drone&last_id=${lastBookingId}`)
      .then(res => res.json())
      .then(data => {
