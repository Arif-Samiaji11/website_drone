...
@section('title', 'Kelola Booking Crews - Admin')

@section('content')
<!-- Leaflet CSS -->
<link class="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" integrity="sha256-p4NxAoJBhIIN+hmNHrzRCf9tD/miZyoHS5obTRR9BMY=" crossorigin="" />
<div class="max-w-6xl mx-auto">
  <div class="mb-6 flex justify-between items-center">
    <div>
      <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Daftar Booking Crews (Photographer / Videographer)</h1>
      <p class="text-sm text-slate-500 mt-1">Daftar pengajuan booking photographer & videographer darat dari client.</p>
    </div>
  </div>
      ...
      <table class="w-full text-left border-collapse text-sm">
        <thead>
          <tr class="bg-slate-50/75 border-b border-slate-200 text-slate-600 font-semibold text-xs uppercase tracking-wider">
            ...
            <th class="px-6 py-4">Tanggal Booking / Lokasi</th>
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse($bookings as $b)
            @include('admin.submissions.partials.booking_crew_row', ['b' => $b])
          @empty
            <tr class="empty-state-row">
              <td colspan="7" class="px-6 py-12 text-center text-slate-500">
                <div class="flex flex-col items-center justify-center gap-2">
                  ...
                  <span class="font-medium">Belum ada pengajuan booking crews.</span>
                </div>
              </td>
            </tr>
          @endforelse
...
<script>
  ...
  // REAL-TIME CHECK FOR NEW CREW BOOKINGS
  let lastBookingId = {{ $bookings->first()->id ?? 0 }};
  const tableBody = document.querySelector('tbody');
  
  setInterval(function() {
    if (lastBookingId <= 0) return;
    
    fetch(`{{ route('admin.submissions.fetch-new') }}?type=booking_crew&last_id=${lastBookingId}`)
      .then(res => res.json())
      .then(data => {
