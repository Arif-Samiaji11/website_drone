...
@section('title', 'Kelola Portofolio Jasa Udara Drone - Edit')
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 max-w-3xl">
  <h1 class="text-xl font-extrabold text-slate-900 mb-4">Kelola Portofolio Jasa Udara Drone - Edit</h1>
  ...
  <form method="POST" action="{{ route('admin.portofolio-udara.update', $portofolio) }}" class="space-y-4">
    @csrf
    @method('PUT')

    <div>
      <label class="text-sm font-semibold text-slate-700">Judul</label>
      <input name="judul" value="{{ old('judul', $portofolio->judul) }}"
    ...
    <div>
      ...
      <textarea name="deskripsi" rows="4"
                class="mt-1 w-full px-3 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-red-200">{{ old('deskripsi', $portofolio->deskripsi) }}</textarea>
    </div>

    <div class="grid md:grid-cols-2 gap-4">
      <div>
        <label class="text-sm font-semibold text-slate-700">Lokasi</label>
        <input name="lokasi" value="{{ old('lokasi', $portofolio->lokasi) }}"
               class="mt-1 w-full px-3 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-red-200">
      </div>
      ...
      <div class="mt-1 bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
        <div class="grid sm:grid-cols-3 gap-3">
          ...
          <div class="sm:col-span-2">
            ...
            <input type="text" name="cover" id="cover-url-input" value="{{ old('cover', $portofolio->cover) }}"
