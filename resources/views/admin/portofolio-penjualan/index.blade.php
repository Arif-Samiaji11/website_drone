...
@section('title', 'Kelola Portofolio Penjualan')

@section('content')
<div class="flex items-center justify-between mb-4">
  <div>
    <h1 class="text-xl font-extrabold text-slate-900">Kelola Portofolio Penjualan</h1>
    <p class="text-sm text-slate-500">Produk & penjualan</p>
  </div>
  <a href="{{ route('admin.portofolio-penjualan.create') }}"
        ...
        <td class="p-3">
          <div class="flex justify-end gap-2">
            <a class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 transition"
               href="{{ route('admin.portofolio-penjualan.show', $item) }}">Detail</a>
            <a class="px-3 py-1.5 rounded-lg bg-amber-100 hover:bg-amber-200 transition"
               href="{{ route('admin.portofolio-penjualan.edit', $item) }}">Edit</a>
            <form method="POST" action="{{ route('admin.portofolio-penjualan.destroy', $item) }}"
