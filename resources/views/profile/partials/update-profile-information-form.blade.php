<section>
    <header>
        <h2 class="text-lg font-bold text-white tracking-wide">
            {{ __('Profile Information') }}
        </h2>

        <p class="mt-1 vg-desc">
            {{ __("Update your account's profile information and email address.") }}
        </p>
    </header>

    <form id="send-verification" method="post" action="{{ route('verification.send') }}">
        @csrf
    </form>

    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')

        <div>
            <label for="name" class="vg-label">{{ __('Name') }}</label>
            <input 
                id="name" 
                name="name" 
                type="text" 
                class="vg-input mt-1" 
                value="{{ old('name', $user->name) }}" 
                required 
                autofocus 
                autocomplete="name" 
                placeholder="Nama Anda"
            />
            @if ($errors->has('name'))
                @foreach ($errors->get('name') as $msg)
                    <div class="vg-error">{{ $msg }}</div>
                @endforeach
            @endif
        </div>

        <div>
            <label for="email" class="vg-label">{{ __('Email') }}</label>
            <input 
                id="email" 
                name="email" 
                type="email" 
                class="vg-input mt-1" 
                value="{{ old('email', $user->email) }}" 
                required 
                autocomplete="username" 
                placeholder="email@example.com"
            />
            @if ($errors->has('email'))
                @foreach ($errors->get('email') as $msg)
                    <div class="vg-error">{{ $msg }}</div>
                @endforeach
            @endif

            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div class="mt-3">
                    <p class="text-sm text-white/70">
                        {{ __('Your email address is unverified.') }}

                        <button form="send-verification" class="underline text-[#00c7ff] hover:text-white transition focus:outline-none">
                            {{ __('Click here to re-send the verification email.') }}
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 font-semibold text-sm text-emerald-400">
                            {{ __('A new verification link has been sent to your email address.') }}
                        </p>
                    @endif
                </div>
            @endif
        </div>

        <div class="flex items-center gap-4">
            <button type="submit" class="vg-btn-primary">{{ __('Save') }}</button>

            @if (session('status') === 'profile-updated')
                <p
                    x-data="{ show: true }"
                    x-show="show"
                    x-transition
                    x-init="setTimeout(() => show = false, 2000)"
                    class="text-sm text-emerald-400 font-semibold"
                >{{ __('Saved.') }}</p>
            @endif
        </div>
    </form>
</section>
